...
class Remote_Imgur extends Remote
{
    ...
    protected function _doLogin() 
    {
        if( ! $this->get('cookieLogin')) {
            ...
            else if($this->http->getResponseStatus() == 302
                OR stripos($this->http->getResponseCookie(), 'just_logged_in=1') 
                OR (stripos($this->http->getResponseHeader('location'), $this->_username))
            ) {
